        ...
        <div class="page-content">
            <div class="container-fluid">
                ...
                <form action="{{route('drivers.update', $driver->id)}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">{{$driver->title}}</h4>
                            <div class="row">
                                <div class="col-6">

                                    <div class="mb-3">
                                        <label class="col-form-label">Title</label>
                                        <input value="{{$driver->title}}" class="form-control" type="text" name="title">
                                        @if($errors->first('title')) <small class="form-text text-danger">{{$errors->first('title')}}</small> @endif
                                    </div>

                                    <div class="mb-3">
                                        <button class="btn btn-primary">Yadda saxla</button>
                                        <a class="btn btn-primary" href="{{route('drivers.index')}}">Siyahıya qayıt</a>
                                    </div>
                                </div>
                            </div>
                        </div>
